...
const char *MODBUS_TAG = "MASTER_TEST";
...
void *master_get_param_data(const mb_parameter_descriptor_t *param_descriptor)
{
    ...
    {
        ESP_LOGE(MODBUS_TAG, "Wrong parameter offset for CID #%u", (unsigned)param_descriptor->cid);
...
void set_value(int edd_val, int bates_val, uint16_t *val)
{
    ...
    else if (edd_val && !bates_val)
    {
        ...
        {
            ESP_LOGE(MODBUS_TAG, "Error where no switch selection for bates charging is detected.");
        }
    }
    else if (!edd_val && bates_val)
    {
...
void master_operation_func(void *arg, int EDDISON_VAL, int BATES_VAL, OperationType op_type, Queue *queue)
{
    ...
    //    ESP_LOGI(MODBUS_TAG, "Start modbus test...");
            ...
            if ((err != ESP_ERR_NOT_FOUND) && (param_descriptor != NULL))
            {
                ...
                if (op_type == WRITE_DATA && cid == 31)
                {
                    ...
                    if (err == ESP_OK)
                    {
                        ...
                        ESP_LOGI(MODBUS_TAG, "Characteristic #%u %s (%s) value = (0x%" PRIx16 "), write successful.",
                    ...
                    else
                    {
                        ...
                        ESP_LOGE(MODBUS_TAG, "Characteristic #%u (%s) write fail, err = 0x%x (%s).",
                ...
                else if (op_type == READ_MEANWELL && param_descriptor->mb_slave_addr == MEANWELL_ID && reading_from_meanwell)
                {
                    ...
                    if (err == ESP_OK)
                    {
                        ...
                        ESP_LOGI(MODBUS_TAG, "Characteristic #%u %s (%s) value = %.2f (0x%04X) read successful.",
                                 param_descriptor->cid,
                                 param_descriptor->param_key,
                                 param_descriptor->param_units,
                                 scaled_value,
                    ...
                    {
                        ESP_LOGE(MODBUS_TAG, "Characteristic #%u (%s) read fail, err = 0x%x (%s).",
                                 param_descriptor->cid,
                                 param_descriptor->param_key,
                                 (int)err,
                                 (char *)esp_err_to_name(err));
                        ...
                        {
                            ESP_LOGI(MODBUS_TAG, "Timeout error occurred. Skipping all other CID's.");
    ...
    //    if (alarm_state) {
    //        ESP_LOGI(MODBUS_TAG, "Alarm triggered by cid #%u.", param_descriptor->cid);
    //    } else {
    //        ESP_LOGE(MODBUS_TAG, "Alarm is not triggered after %u retries.", MASTER_MAX_RETRY);
    //    }
    //    ESP_LOGI(MODBUS_TAG, "Destroy master...");
